...
func (gn *GossipNetwork) shouldProcessMessage(msg GossipMessage) bool {
	// Check if message is from the same room
	if msg.RoomID != gn.roomID {
		return false
	}
...
func clearBlockchainForRoom(roomID string) error {
	...
	fmt.Printf("📖 Read blockchain data: %d total blocks\n", len(blockchain))
